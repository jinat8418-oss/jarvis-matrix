...
if "global_incident_archive" not in st.session_state:
    st.session_state["global_incident_archive"] = [
        {"timestamp": "2026-05-20 14:22:01", "target": "System Bootstrap Node", "type": "SYSTEM INIT", "status": "CLEARED"}
    ]
...
if module == "🌐 System Mainframe":
    ...
    if st.button("EXECUTE LIVE SIMULATED BREACH INTERCEPT"):
        ...
        # APPEND NEW REAL-TIME EVENT DIRECTLY TO SYSTEM STATE LOGS
    ...
    # --- THE SYSTEM HISTORIC LOG HISTORY PANEL ---
    st.markdown("---")
    st.markdown("### 🗃️ CHRONOLOGICAL SYSTEM THREAT LOG ARCHIVE")
    st.write("Below is the historical stream of intercepts recorded across this current operating runtime session:")
    
    # Render historical logs as an organized markdown block table
